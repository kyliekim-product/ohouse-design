...
interface ChildrenMixin {
   ...
   *
   * **Instance 제약**: InstanceNode 또는 그 하위 노드에는 appendChild 불가 ("Cannot move node. New parent is an instance or is inside of an instance").
   * 단, {@link SlotNode}에는 가능. 컴포넌트에 {@link ComponentNode.createSlot}으로 SLOT을 만들면, 인스턴스 내 해당 SlotNode에 자유롭게 자식을 추가할 수 있다.
   */
...
interface ComponentNode
  ...
  /**
   * Creates a SlotNode as a child of this component. A slot is a placeholder container
   * that allows freeform content editing in instances via appendChild/insertChild.
   * Also creates a corresponding SLOT property in componentPropertyDefinitions.
   *
   * @param name - The name of the slot property.
   * @returns The newly created SlotNode.
   *
   * @see https://developers.figma.com/docs/plugins/api/SlotNode
   */
  createSlot(name: string): SlotNode
  /**
...
/**
 * A SlotNode represents a slot within a component or instance.
 * A slot is a child frame of a component that has freeform content editing.
 * In instances, SlotNode accepts appendChild/insertChild to add custom content.
 *
 * Created via {@link ComponentNode.createSlot}.
 *
 * @see https://developers.figma.com/docs/plugins/api/SlotNode
 */
interface SlotNode
  ...
  readonly type: 'SLOT'
  /**
   * Resets a slot node to the original component slot content,
   * removing all override children added to the instance slot.
   */
  resetSlot(): void
}
